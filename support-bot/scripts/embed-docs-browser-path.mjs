...
if (qi >= 0) {
  ...
  const raw = JSON.parse(fs.readFileSync("data/chunks.json", "utf8"));
  // build_chunks.py 가 main 이 아닌 곳에서 만들었으면 출처 URL 이 아직 열리지 않는다.
  // 그대로 벡터스토어를 만들면 고객이 여는 링크가 404 가 된다.
  if (!Array.isArray(raw)) {
    throw new Error(
      `청크가 ${raw.pendingRef} 에서 만들어졌다. 출처 URL 은 main 을 가리키는데 아직 거기 없다.\n` +
      `  도움말이 main 에 머지된 뒤 python3 scripts/build_chunks.py 를 다시 돌린다.`,
    );
  }
  const chunks = raw;
  const rows = [];
  for (const c of chunks) {
    ...
    // id, text, url, doc, section 을 보존하고 vector 만 더한다.
    // doc 은 화면에 「위시리스트 · 샀을 때 옮기기」로 보여 줄 사람이 읽는 이름표다.
    rows.push({ id: c.id, text: c.text, url: c.url, doc: c.doc, section: c.section, vector });
  ...
  if ([...keys].sort().join(",") !== "doc,id,section,text,url,vector") throw new Error(`필드가 다르다: ${[...keys]}`);

  const oi = process.argv.indexOf("--out");
  const dest = oi >= 0 ? process.argv[oi + 1] : "app/public/help-docs.json";
  ...
  if (oi < 0) {
    const ANCHOR = "위시에 담아둔 걸 샀는데 어떻게 있템으로 옮겨요?";  // 대조용 고정 문장
    fs.writeFileSync(
  ...
  console.log(`  필드 보존         : id, text, url, doc, section + vector`);
